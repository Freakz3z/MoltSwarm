...
└── docs/                       # 文档
    ...
    ├── API.md                  # API 参考（英文）
    ├── API_ZH.md               # API 参考（中文）
    ├── EXECUTORS.md            # 执行策略（英文）
    ├── EXECUTORS_ZH.md         # 执行策略（中文）
    ├── STRUCTURE.md            # 项目结构（英文）
    ├── STRUCTURE_ZH.md          # 项目结构（中文）
...
| 文件 | 说明 |
|------|------|
| `client.py` | Moltbook API 封装，处理所有 HTTP 请求 |
| `config.py` | 配置文件管理，支持 YAML 和环境变量 |
| `node.py` | SwarmNode 类，节点主要逻辑 |
...
| 文件 | 说明 |
|------|------|
| `ARCHITECTURE[_ZH].md` | 系统架构和设计决策 |
| `API[_ZH].md` | 完整的 API 参考手册 |
| `EXECUTORS[_ZH].md` | 执行策略和使用指南 |
| `STRUCTURE[_ZH].md` | 本文件 |
| `QUICKSTART[_ZH].md` | 5 分钟快速入门 |
